...
+------------------------------v------------------------------+
|                     Application / Domain                    |
|           (Project, Task, Activity Log, Sync Engine)        |
+--------------+------------------------------+---------------+
...
### (1) Peer Discovery Port (ノード発見ポート)
...
### (2) Peer Transport Port (P2P通信ポート)
...
### (3) Project Repository Port (ストレージポート)
...
- ドメインコアは具象ソケット通信や特定のDBドライバを `new` せず、すべてインターフェース経由で外部注入（DI）を受ける。
- テスト時には Mock / In-Memory アダプターを挿入可能とし、実ネットワークを起動せず高速にドメインロジック検証を実施可能にする。
