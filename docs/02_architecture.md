...
- **機能**:
  - **ノードダブルクリック最上部頂点化 ＆ サブツリー限定表示 (Subtree Focus Mode)**: ダブルクリック時にクリックした要素を画面最上部（ルート頂点）として固定配置し、親が存在する場合は上方向へと接続線を伸ばして視覚表示。対象要素と配下子要素群のみが存在するかのように他ノードを非表示化。
  - **相対ネスト表示適応 (Relative Nest Filtering)**: サブツリーフォーカス時、ネスト切替（1階層 | 2階層 | すべて）をフォーカスノード起点で相対的に適応。
  - **インスペクターノード干渉・重なり防止 (Inspector Shielding & Layout Isolation)**: タスク選択時にドロワー・パネルと子要素ラベルや接続線が重なって隠してしまう問題を物理力場拡張 ＆ Z-indexシールドで防止。
