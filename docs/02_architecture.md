...
- **機能**:
  - **初期配置の絡まり防止アルゴリズム (Initial Untangling)**: ツリー階層・分岐角度に基づく放射状反発初期配置により、初期描画時のノード重なりや文字の絡まりを根本排除。
  - **階層型Y軸重力 (Hierarchical Gravity)**: 親の親（上位祖先）ほど自然と画面上部へ移動し、配下の子ノードは下方向へ浮遊配置される物理バイアス。
  - **サブツリー強調 ＆ 文字重なり防護 (Subtree Focus & Label Protection)**: ノード選択時、対象ノードおよび配下の子孫タスク群（子・孫ノード）とリンクをハイライトし、無関係ノードを半透明化。テキストラベル背景にアウトラインプレートを形成し文字重なりを防護。
  - **画面空白クリックによるフォーカス解除**: ノード以外のキャンバス空白部クリックで選択状態をクリアし初期表示へ復帰。
  - **画面端 ネスト表示深度コントロール (Edge Nest Depth Filter)**: 画面端に配置されたコントローラーから表示階層範囲（デフォルト: すべて `'all'`）を切り替え。
